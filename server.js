...
app.get("/leagues", async (req, res) => {
  try {
    ...
    if (data.response === 0) {
      res
        .status(404)
        .send("Não foram encontradas ligas para o país especificado");
    } else {
      res.json(data);
    }
  } catch (error) {
...
app.get("/teams", async (req, res) => {
    try {
        const apiLeague = req.query.api_league;
        const apiSeason = req.query.api_season;
        ...
        if(data.response === 0) {
            res.status(404).send('Não foram encontrados times para a liga e temporada especificadas');
        } else {
            res.json(data);
        }
    } catch(error) {
